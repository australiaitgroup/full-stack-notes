...
- 语法：
  ![css syntax](../images/css_syntax.gif)
...
- 行内块元素（inline-block）：可以通过 CSS 设置宽高。
  ![inline and block](../images/css_inline_and_block.jpg)
...
![CSS盒模型](../images/css_box_model.png)
...
![flex conatiner](../images/css_flex_container.svg)
![flex items](../images/css_flex_items.svg)
...
- 可以使用` flex-direction`属性调节主轴的方向，属性值包括：`row`, `row-reverse`, `column`, `column-reverse`
  ![flex direction](../images/css_flex_direction.svg)

主轴是否自动换行

- 使用`flex-wrap`属性来决定主轴是否自动换行，属性值包括: `nowrap`, `wrap`, `wrap-reverse`.
  ![flex wrap](../images/css_flex_wrap.svg)
  主轴 flex item 分布方式
- 使用`justify-content`调节，如：`center`, `space-between`, `space-around`等
  ![justify content](../images/css_justify_content.svg)
